...
export const operacionCompra = async (req, res) => {
  ...
  const id_comprador = 125;
  const importe = 1000;
  const id_vendedor = 114;
  const id_producto = 16;

  //------ DATOS DE LA COMPRA
  var post_usuario = [ importe, id_comprador, id_vendedor, 1 ];
  ...
  console.log({ query: query2, variables: { id_vendedor: id_vendedor } });
  const resultado2 = await query(query2, id_vendedor);

  console.log(resultado2)

  //------- DATOS DEL PRODUCTO
  var query3 = "SELECT * FROM productos WHERE id_producto = ?;";
  console.log({ query: query3, variables: { id_producto: id_producto } });
  const resultado3 = await query(query3, id_producto);
